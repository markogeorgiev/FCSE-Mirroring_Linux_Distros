...
if [[ "$MIRROR_NAME" == "rocky" ]]; then
  ...
  echo "Syncing Rocky Linux 8.10..." >> /var/log/sync.log
  rsync -avz --delete "$ROCKY_BASE/8.10/" "$MIRROR_DIR/8.10/" >> /var/log/sync.log 2>&1

  echo "Syncing Rocky Linux 9.6..." >> /var/log/sync.log
  rsync -avz --delete "$ROCKY_BASE/9.6/" "$MIRROR_DIR/9.6/" >> /var/log/sync.log 2>&1

  echo "Syncing Rocky Linux 10.0..." >> /var/log/sync.log
  rsync -avz --delete "$ROCKY_BASE/10.0/" "$MIRROR_DIR/10.0/" >> /var/log/sync.log 2>&1

  echo "[$(date)] sync.sh finished for rocky" >> /var/log/sync.log
  exit 0
fi
